...
//   init: function() {
//       $('.js-product-slider2 .products-slider').each(function(index, value) {
...
var review_message = $('#stamped-reviews-widget').find('.stamped-reviews-message');
window.ProductReviewSlider = {
  init() {
    $('.js-product-review-slider').flickity({
      autoPlay: 3000,
...
window.ProductCartSlider = {
  init() {
    $('.js-product-cart-slider').flickity({
      autoPlay: false,
      pauseAutoPlayOnHover: true, 
      wrapAround: true,
      lazyLoad: true,
...
window.ArticleSlider = {
  init() {
    $('.js-article-slider .article-slider').flickity({
      autoPlay: false,
      pauseAutoPlayOnHover: true,
      wrapAround: true,
      lazyLoad: true,
...
window.CollectionSlider = {
  init() {
    $('.js-collection-slider .collection-slider').flickity({
      autoPlay: 3000,
      pauseAutoPlayOnHover: true,
      wrapAround: true,
      lazyLoad: true,
...
window.VideoSlider = {
  init() {
    $('.js-video-slider .video-slider').flickity({
      autoPlay: 3000,
      pauseAutoPlayOnHover: true,
      wrapAround: true,
      lazyLoad: true,
...
window.selectCallback = function (variant, selector, $element) {
  ...
  //rs: Scroll to correct variant image
  if (variant && variant.featured_image && $product.is(':visible')) {
    ...
    $sliders.each((_, slider) => {
      ...
      const $selectedVariant = variant.featured_media.alt
      const $slides = $slider.find('.gallery-cell');     

      const $thumbnailProductGallery = $slider.closest('.js-product-section').find('.product_gallery_nav');
      ...
      if ($slider && $sliderInstance !== undefined) {       
        ...
        //const $selectedSlide =  window.Flickity.data($slider[0]).selectedElement.data('product-id');       
       
        //assign image to selected variants
        $sliderInstance.select(index, false, true);


        //scroll to main image   
        
        const $selectedSlide = $('.gallery-type--product').find('.gallery-cell.is-selected');

        if($selectedSlide.length > 0){
          //console.log('====test1');
          $('html,body').animate({ 
            scrollTop: $selectedSlide.offset().top - 80
          }, 'slow'); 
        }else{
          //rs:fix image glitch not showing on popup
        ...
        //console.log('======selectedColor:'+$currentShoppableImage);
       /* console.log('=====index slide:'+index);  
        console.log('=====img:'+variant.featured_image.id);
        console.log('=====title:'+variant.title);
        console.log('=====alt:'+$selectedVariant); 
        console.log('=====variant1:'+variant.option1); 
        console.log('=====variant2:'+variant.option2); */      
  ...
  if (variant && variant.available === true) {
    ...
    const message = variant ? window.PXUTheme.theme_settings.sold_out_text : window.PXUTheme.translation.unavailable_text;
...
window.productPage = {
  ...
  enableGallery(selector) {
    ...
    const product_review = $('.product-review');
    const five_icons = $('.five-icons');
    ...
    if (zoom === true) {
      ...
      //rs: only enable zoom for mobile devices
    ...
    //hide Thumbnails when scroll bottom   
    five_icons.waypoint(function (direction) {
     if (direction === 'down') {      
        $thumbnailProductGallery.hide();
     }
     else {
        $thumbnailProductGallery.show();
     }      
   }, { offset: '70%' });
    ...
    $productGallery.on('ready.flickity', () => {
      ...
      //rs: autoplay video
      let $videoSlide = $slides.find('.product-gallery__video');
      $videoSlide.waypoint(function (direction) {
        if (direction === 'down') {
          $videoSlide.parent('.gallery-cell').addClass('is-selected'); 
        }
        else {
          $videoSlide.parent('.gallery-cell').removeClass('is-selected'); 
        }
        checkForVideos();     
      }, { offset: '50%' });
    ...
    //rs: Main Desktop Slider
    $productGallery.flickity({
      wrapAround: true,
      adaptiveHeight: true,
      dragThreshold: 10,
      imagesLoaded: true,
      //pageDots: $productGallery.data('media-count') > 1 || $slides.length > 1,
    ...
    //rs: Add events when thumbnails is clicked
    var image_viewport = $('.js-product-gallery').find('.flickity-viewport');
    if (thumbnailsEnabled === true && thumbnailsSliderEnabled === true && $slides.length > 1) {

      var sliderHeight = $slides.height() - 70;           
      ...
      if (window.PXUTheme.media_queries.large.matches) {
        ...
        if (thumbnailsPosition === 'left' || thumbnailsPosition === 'right') {
          ...
          $thumbnails.on('click', event => {
            ...
            //scroll to Image Wrapper         

            $('html,body').animate({ 
              scrollTop: $('#slide_'+index+'').offset().top + sliderHeight
            }, 'slow');
            
            //image_viewport.css({transform: 'translateY("-200px") !important'});
            //image_viewport.addClass('tesssss');
          ...
          $productGallery.on('select.flickity', (event, index) => {
            //rs: set selected nav cell
...
class NavigationDesktopParent {
  constructor(options) {
    ...
    this.mainNav = document.querySelector('.main-nav__wrapper');
    ...
    this.mouseover = () => {
      ...
      document.body.classList.add('is-dropdown');
      document.querySelector('.nav-desktop__tier-2').classList.add('submenu-open');
      this._open();
    ...
    this.mouseout = () => {  
      document.body.classList.remove('is-dropdown');
      document.querySelector('.nav-desktop__tier-2').classList.remove('submenu-open');
  ...
  _open(force = false) {
    ...
    this.animation.animateTo('open', {
      ...
      if (state == 'open') {
        ...
        //document.body.classList.add('is-dropdown');
        //this.mainNav.remove
        //console.log(this.mainNav.getAttribute('class'));

        //rs: show or hide sticky nav on hover
        //this.mainNav.classList.add('test');
...
class MobileMenuDrawer {
  constructor(el) {
    ...
    this.close_icon = this.el.querySelector('[data-mobile-menu-close]');
    ...
    this.events.register(this.close_icon, 'click', e => {
      e.preventDefault();
      this.close();     
    });
  }
      ...
      if ($('.header').hasClass('header-fixed--true')){
        ...
        if (!$('.main-nav__wrapper').hasClass('sticky_nav')) {
          ...
          window.sticky_nav = new Headhesive('.main-nav__wrapper', {
            ...
            onInit: function() {              
              $('.sticky_nav .secondary_logo').css('display', 'block');
...
customElements.define(ProductCTA.shortcode, ProductCTA);
 ...
 function hideSelectedImageVariant(){

  console.log('=======click variant');
    $('.selectv').click(function(){
      var color = $(this).val(),     
          swatch_selected = $(this).data('swatch-selected'),
          swatch_color_size = $(this).data('color-size'),
          selected_color = $('.selected-color'),
          selected_size = $('.selected-size');     
      
      //append selected Color values and show selected images
      
      //hide gallery and thumbnails by default
      if(swatch_selected == "color"){
        selected_color.html(color);    
      }else{
        selected_size.html(color);
      }

      //if product has color and sizes
      //console.log('===clicked swatches');
      //console.log('===swatch_selected:'+swatch_selected);

    });    

    //if (window.matchMedia("(min-width: 769px)").matches){
    var selected_wrapper = $('select[name="id"]');     
    selected_wrapper.on('change',function(){
      //console.log('=======changed variant');

      var selected_variant = $(this).find(":selected").text(),          
          selected_variant_filter = selected_variant.replace('/',('+'))
          slide = $('.js-product-gallery').find('.gallery-cell'),
          slide_nav = $('.product-gallery__thumbnails').find('.gallery-cell'), 
          selected_slide = $('.js-product-gallery').find('.gallery-cell.is-selected');


      if(selected_variant !== 'Default Title'){
          //Main Slider
          slide.hide();
          ...
          slide_nav.hide();  
        // selected_slide.show();
          $('.js-product-gallery').find(`.gallery-cell[data-title*='${selected_variant_filter}']`).show();  
          $('.product-gallery__thumbnails').find(`.gallery-cell[data-title*='${selected_variant_filter}']`).show();      
      }      
       
      
      console.log('===========Filter Value1:'+ selected_variant_filter);
      
      //rs: Main Gallery Mobile
      if (window.is_touch_device() && window.PXUTheme.media_queries.medium.matches ||  window.PXUTheme.media_queries.large.matches && !window.location.href.includes("/products/")) {
        const $mainGallery = $('.js-product-gallery').flickity();
        const $slides = $mainGallery.find('.gallery-cell');     
        var selectedVariant = '[data-title="'+selected_variant_filter+'"]';
        ...
        if ( isFlickity ) {
          ...
          // init new Flickity
          //$mainGallery.prototype._touchActionValue = 'pan-y pinch-zoom';
        ...
        if(selected_variant !== 'Default Title'){
          $selectedVariant = '[data-title="'+selected_variant_filter+'"]';
        }else{
          $selectedVariant = "";
        }
        
        //console.log('======selected var'+$selectedVariant);
        $mainGallery.flickity({
          cellSelector: $selectedVariant,
          initialIndex: 0, 
          dragThreshold: 20,
          wrapAround: true,
          adaptiveHeight: true,      
          imagesLoaded: true,
          pageDots: $mainGallery.data('media-count') > 1 || $slides.length > 1,
          //prevNextButtons: $mainGallery.data('media-count') > 1 || $slides.length > 1,         
          prevNextButtons: true,
          autoPlay:false, 
          //autoPlay: slideshowSpeed * 1000,
          watchCSS: false,
      ...
      //console.log('========'+window.PXUTheme.media_queries.medium.matches);
    });     
    //}
 }

 function hideSelectedImageVariantMobile(){
  ...
  $('.selectv').click(function(){
    ...
    console.log('===========Filter Value2:' +color + "Variant:" + selected_variant_filter);
    
    //append selected Color values
    if(swatch_selected == "color"){
      selected_color.html(color);
    }else{
      selected_size.html(color);
  ...
  //  $('.js-product-gallery').find(`.gallery-cell`).hide();
    $('.product-gallery__thumbnails').find(`.gallery-cell`).hide();   
    //Find class contains selected colors
    //$('.js-product-gallery').find(`.gallery-cell[data-title*='${color}']`).show();   
    $('.product-gallery__thumbnails').find(`.gallery-cell[data-title*='${color}']`).show();
  
  });  
  
  setTimeout(function(){   
    $('.product-gallery__thumbnails').removeClass('hide--thumbs');
     $('.selectv:checked').trigger('click');
     console.log('trigger');
  },500); 
 ...
 if (window.PXUTheme.media_queries.large.matches && currentURL.includes("product")) {    
  setTimeout(function(){   
      //$('.product-gallery__thumbnails').removeClass('hide--thumbs');
      if ($('.selectv:checked').length > 0 ){
        $('.selectv:checked').trigger('click');

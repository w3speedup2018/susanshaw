...
setTimeout(function(){
  ...
  window.ArticleSlider.init();
  window.CollectionSlider.init();
  window.VideoSlider.init();
  window.ProductReviewSlider.init();
  window.ProductCartSlider.init();
  ...
  window.utils.createAccordion('.footer_menu', 'h4', 'ul');
  window.utils.createAccordion('.footer_content', 'h4', 'div.toggle_content');
  ...
  function refreshCart(cart) {   
    ...
    const shippingTotal =  10000;
    const progress = (cart_total / shippingTotal)*100;
    const total_discount = cart.total_discount;
    const amount_remaining = shippingTotal - cart_total;


    let shippingText = $('.shipping-text');
    let shipping_wrapper = $('.free-shipping-wrapper');
    let shipping_bar =  $('.free-shipping-wrapper');   
            
    //console.log(`Shipping Threshold: ${shippingTotal}`);
    //console.log(`Progress: ${progress}`);
    //console.log(`Amount remain: ${amount_remaining}`);
    console.log(`Total: ${cart_total}`);
    //console.log(`Total Discount: ${total_discount}`);
  
    
    if(cart_total < shippingTotal ){      
      cartShippingBarHTML += `<span class="shipping-progress"> You're ${window.Shopify.formatMoney(amount_remaining, $('body').data('money-format'))} away from Free Shipping</span>`;
      $('.shipping-messages').html(cartShippingBarHTML);    

      shippingText.find('.shipping').removeClass('hide');      
      shippingText.find('.free-shipping').addClass('hide');      
      shipping_wrapper.removeClass('hidden free-shipping');
      shipping_wrapper.find('.filling').removeClass('all-the-way');         
      shipping_wrapper.find('.filling').css('width', progress + '%')
      
    }else{ 
      cartShippingBarHTML += `<span class="shipping-free">You've earn complimentary free shipping!</span>`;    
      $('.shipping-messages').html(cartShippingBarHTML);      

      shippingText.find('.free-shipping').removeClass('hide');      
      shippingText.find('.shipping').addClass('hide');        
      shipping_wrapper.addClass('free-shipping');
      shipping_wrapper.find('shipping-free').addClass('hidden');
      shipping_wrapper.find('.filling').addClass('all-the-way');
    } 
    ...
    if (cart.item_count === 0) {
      ...
      shipping_wrapper.addClass('hidden');
    } else {
      shipping_wrapper.removeClass('hidden');
     ...
     for (let i = 0; i < cartDiscounts.length; i++) {
        const discount_application = cartDiscounts[i].discount_application || {};
        const _amount = discount_application.total_allocated_amount || cartDiscounts[i].total_allocated_amount;
        const amount = window.Shopify.formatMoney(_amount, $('body').data('money-format')) ;
        const total_discount = cart.total_discount;
        const title = discount_application.title || cartDiscounts[i].title; // Provide a default value if title is undefined       
  
        //console.log(`Label: ${title}`);
        //console.log('Dicount Amount:' + amount);       
      } */

      /* Get First Cart Discount */
      const cartDiscounts = cart.cart_level_discount_applications;
      if (cartDiscounts.length > 0) {
          const firstDiscount = cartDiscounts[0];
          const discount_application = firstDiscount.discount_application || {};
          const _amount = discount_application.total_allocated_amount || firstDiscount.total_allocated_amount;
          const amount = window.Shopify.formatMoney(_amount, $('body').data('money-format'));
          const total_discount =   window.Shopify.formatMoney(cart.total_discount, $('body').data('money-format'));
          const title = discount_application.title || firstDiscount.title; // Provide a default value if title is undefined

          cartDiscountsHTML += `<span class="cart_discounts--title text-left"><strong>Discount:</strong></span>`;
          cartDiscountsHTML += `<span class="cart_discounts--title text-right text-trasnsform">${title}</span>`;
          cartDiscountsHTML += '<span class="cart_discounts--price">';
          cartDiscountsHTML += `-<span class="money">${total_discount}</span></span>`;
     
          //console.log(`discount_name: ${title}`);
          //console.log(`total_discount_amount: ${total_discount}`);     
      }   
          ...
          $.ajax({
            ...
            success(cart) {
              setTimeout(function () {
                ...
                } else if ($('.main-nav__wrapper').length) {
                  $('.main-nav__wrapper .cart-container').addClass('active_link');               

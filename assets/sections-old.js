  ...
  if (variant && variant.featured_image && $product.is(':visible')) {
    ...
    $sliders.each((_, slider) => {
      ...
      if ($slider && $sliderInstance !== undefined) {       
        ...
        $slides = $slider.find('.gallery-cell');
        const sliderHeight = $slides.height() - 70;      
       /* $('html,body').animate({ 
      ...
      if (window.PXUTheme.media_queries.large.matches) {
        ...
        if (thumbnailsPosition === 'left' || thumbnailsPosition === 'right') {
          ...
          $thumbnails.on('click', event => {
            ...
            const sliderHeight = $slides.height() - 70;           
            console.log('=========slide height:'+sliderHeight);
            $('html,body').animate({ 
              scrollTop: $('#slide_'+index+'').offset().top + sliderHeight
            }, 'slow');
...
function buildActiveTags(){
  ...
  console.log('tags built');
    var col_link = $(".breadcrumb_link").attr("href");

    var activeTags = $('.filter-active-tag');
    ...
    var currentSelection = $(`
			<div class="clear-tags_container">
              ...
              <a id="clear__all" href="${col_link}">Clear Filters</a>
			</div>`)
    
    
    if($('#current__selection').length < 1 ){
      currentSelection.appendTo('#clear-tags-list');
    }

    activeTags.each(function(i,v){
    
      var item = $(v);
      //console.log(item);
      if(item.find('li').length > 0){     
        //console.log('Has actual tag');
        item.clone().insertAfter('#current__selection');      
      }
      
    });
  
  var tabLink = $('.tablinks.active');
  var tabContent = tabLink.siblings('.tabcontent');
  console.log(tabContent);
  var height = tabContent.height();
  //Make the overall filter the correct height otherwise clipping occurs.
  var sidebar = tabContent.closest('.sidebar-wrap');
  sidebar.height(50 + height);
...
$(document).ready(function(){   
  $('.selectv').click(function(){
    ...
    console.log('===========Filter Value:' +color + "Swatch:" + swatch_selected);
